...
pub struct S010AllUniqueSingletons {
    scale: ScaleState,
}

impl Default for S010AllUniqueSingletons {
    fn default() -> Self {
        Self::new()
    }
}

impl S010AllUniqueSingletons {
    pub fn new() -> Self {
        Self {
...
impl Scenario for S010AllUniqueSingletons {
    fn id(&self) -> &'static str {
        "s010-all-unique-singletons"
    }

    fn description(&self) -> &'static str {
        "Every formula is structurally unique, exercising the legacy path."
    }

    fn tags(&self) -> &'static [ScenarioTag] {
        &[ScenarioTag::LegacyOnly, ScenarioTag::SingleCellEdit]
    }
    ...
    fn build_fixture(&self, ctx: &ScenarioBuildCtx) -> Result<ScenarioFixture> {
        ...
        write_workbook(&path, |book| {
            ...
            for r in 1..=rows {
                ...
                let formula = match r % 4 {
                    0 => format!("=A{r}+{r}"),
                    1 => format!("=A{r}*{r}"),
                    2 => format!("=A{r}-{r}"),
                    _ => format!("=A{r}/({r}+1)"),
                };
                sh.get_cell_mut((2, r)).set_formula(formula);
            }
        });
        Ok(ScenarioFixture {
            path,
        ...
        if has_evaluated_formulas(phase) {
            ...
            for row in 1..=rows {
                ...
                invariants.push(ScenarioInvariant::CellEquals {
                    ...
                    row,
                    col: 2,
                    expected: numeric(unique_value(row, a)),
                });
            }
        }
        invariants
...
fn unique_value(row: u32, a: f64) -> f64 {
    match row % 4 {
        0 => a + row as f64,
        1 => a * row as f64,
        2 => a - row as f64,
