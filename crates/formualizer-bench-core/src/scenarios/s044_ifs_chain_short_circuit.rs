...
pub struct S044IfsChainShortCircuit {
    scale: ScaleState,
}

impl Default for S044IfsChainShortCircuit {
    fn default() -> Self {
        Self::new()
    }
}

impl S044IfsChainShortCircuit {
    pub fn new() -> Self {
        Self {
...
impl Scenario for S044IfsChainShortCircuit {
    fn id(&self) -> &'static str {
        "s044-ifs-chain-short-circuit"
    }

    fn description(&self) -> &'static str {
        "IFS family where signed inputs select before an erroring fallback branch."
    }
        ...
        write_workbook(&path, |book| {
            ...
            for row in 1..=rows {
                ...
                sheet.get_cell_mut((2, row)).set_formula(format!(
                    "=IFS(A{row}>0,A{row}*2,A{row}<0,A{row}*3,A{row}=0,0,TRUE,1/0)"
                ));
            }
        });
        Ok(ScenarioFixture {
    ...
    fn invariants(&self, phase: ScenarioPhase) -> Vec<ScenarioInvariant> {
        ...
        if has_evaluated_formulas(phase) {
            ...
            for row in [1, rows / 2, rows] {
                let a = a_at(row, cycles);
                invariants.push(ScenarioInvariant::CellEquals {
                    sheet: "Sheet1".to_string(),
                    row,
                    col: 2,
                    expected: numeric(ifs_value(a)),
                });
            }
        }
        invariants
...
fn edited_row(cycle: usize) -> u32 {
    ((cycle * 1_733) % ROWS as usize) as u32 + 1
}

fn edited_value(cycle: usize) -> f64 {
    match cycle % 3 {
        0 => 30_000.0 + cycle as f64,
        1 => -30_000.0 - cycle as f64,
        _ => 0.0,
    }
}
