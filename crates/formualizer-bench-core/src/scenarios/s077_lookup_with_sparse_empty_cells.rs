...
pub struct S077LookupWithSparseEmptyCells {
    scale: ScaleState,
}
impl Default for S077LookupWithSparseEmptyCells {
    fn default() -> Self {
        Self::new()
    }
}
impl S077LookupWithSparseEmptyCells {
    pub fn new() -> Self {
        Self {
...
impl Scenario for S077LookupWithSparseEmptyCells {
    fn id(&self) -> &'static str {
        "s077-lookup-with-sparse-empty-cells"
    }
    fn description(&self) -> &'static str {
        "VLOOKUP exact-match with sparse empty cells in the lookup column."
    }
...
fn non_gap(mut key: u32) -> u32 {
    while key.is_multiple_of(17) {
        key += 1;
    }
    key
}
fn key_for(row: u32) -> f64 {
    non_gap((row - 1) % DISTINCT_KEYS + 1) as f64
}
fn edited_key(cycle: usize) -> f64 {
    non_gap((cycle as u32 * 17) % DISTINCT_KEYS + 1) as f64
}
