...
pub struct S066XlookupSearchMode2Exact {
    scale: ScaleState,
}

impl Default for S066XlookupSearchMode2Exact {
    fn default() -> Self {
        Self::new()
    }
}

impl S066XlookupSearchMode2Exact {
    pub fn new() -> Self {
        Self {
...
impl Scenario for S066XlookupSearchMode2Exact {
    fn id(&self) -> &'static str {
        "s066-xlookup-search-mode-2-exact"
    }

    fn description(&self) -> &'static str {
        "XLOOKUP exact family with search_mode=2 over a sorted lookup array."
    }
        ...
        write_workbook(&path, |book| {
            ...
            for row in 1..=rows {
                ...
                sheet.get_cell_mut((2, row)).set_formula(format!(
                    "=XLOOKUP(A{row}, Lookup!$A$1:$A$1000, Lookup!$B$1:$B$1000, \"NF\", 0, 2)"
                ));
            }
        });
        Ok(ScenarioFixture {
...
fn edited_key(cycle: usize) -> f64 {
    ((cycle as u32 * 113 + 23) % LOOKUP_ROWS + 1) as f64
}
