...
pub struct S005LongChainFamily {
    scale: ScaleState,
}

impl Default for S005LongChainFamily {
    fn default() -> Self {
        Self::new()
    }
}

impl S005LongChainFamily {
    pub fn new() -> Self {
        Self {
...
impl Scenario for S005LongChainFamily {
    fn id(&self) -> &'static str {
        "s005-long-chain-family"
    }

    fn description(&self) -> &'static str {
        "Internal-dependency chain where each row reads the previous row."
    }

    fn tags(&self) -> &'static [ScenarioTag] {
        &[
            ScenarioTag::LongChain,
    ...
    fn build_fixture(&self, ctx: &ScenarioBuildCtx) -> Result<ScenarioFixture> {
        ...
        write_workbook(&path, |book| {
            ...
            for r in 2..=rows {
                let prev = r - 1;
                sh.get_cell_mut((1, r)).set_formula(format!("=A{prev}+1"));
            }
        });
        Ok(ScenarioFixture {
            path,
        ...
        if has_evaluated_formulas(phase) {
            ...
            for row in 1..=rows {
                invariants.push(ScenarioInvariant::CellEquals {
                    ...
                    row,
                    col: 1,
                    expected: numeric(a1 + (row - 1) as f64),
                });
            }
        }
        invariants
