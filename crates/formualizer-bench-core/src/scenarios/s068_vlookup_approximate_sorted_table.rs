...
pub struct S068VlookupApproximateSortedTable {
    scale: ScaleState,
}

impl Default for S068VlookupApproximateSortedTable {
    fn default() -> Self {
        Self::new()
    }
}

impl S068VlookupApproximateSortedTable {
    pub fn new() -> Self {
        Self {
...
impl Scenario for S068VlookupApproximateSortedTable {
    fn id(&self) -> &'static str {
        "s068-vlookup-approximate-sorted-table"
    }

    fn description(&self) -> &'static str {
        "VLOOKUP approximate TRUE family against a sorted two-column table."
    }
    ...
    fn build_fixture(&self, ctx: &ScenarioBuildCtx) -> Result<ScenarioFixture> {
        ...
        write_workbook(&path, |book| {
            ...
            for row in 1..=rows {
                ...
                sheet
                    .get_cell_mut((2, row))
                    .set_formula(format!("=VLOOKUP(A{row}, Lookup!$A$1:$B$1000, 2, TRUE)"));
            }
        });
        Ok(ScenarioFixture {
            path,
...
fn apply_edit(wb: &mut Workbook, cycle: usize) -> Result<&'static str, anyhow::Error> {
    ...
    Ok("vlookup_approx_key")
}

fn initial_key(row: u32) -> f64 {
    ((row - 1) % LOOKUP_ROWS + 1) as f64 + 0.75
}

fn edited_key(cycle: usize) -> f64 {
    ((cycle as u32 * 113 + 31) % LOOKUP_ROWS + 1) as f64 + 0.75
}
...
fn approx_position(key: f64) -> u32 {
    key.floor().clamp(1.0, LOOKUP_ROWS as f64) as u32
}
