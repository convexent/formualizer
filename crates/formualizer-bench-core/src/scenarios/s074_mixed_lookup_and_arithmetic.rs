...
pub struct S074MixedLookupAndArithmetic {
    scale: ScaleState,
}
impl Default for S074MixedLookupAndArithmetic {
    fn default() -> Self {
        Self::new()
    }
}
impl S074MixedLookupAndArithmetic {
    pub fn new() -> Self {
        Self {
...
impl Scenario for S074MixedLookupAndArithmetic {
    fn id(&self) -> &'static str {
        "s074-mixed-lookup-and-arithmetic"
    }
    fn description(&self) -> &'static str {
        "VLOOKUP exact-match nested inside arithmetic."
    }
        ...
        write_workbook(&path, |book| {
            ...
            for r in 1..=rows {
                ...
                sheet.get_cell_mut((2, r)).set_formula(format!(
                    "=VLOOKUP(A{r}, Lookup!$A$1:$B${LOOKUP_ROWS}, 2, FALSE)*2+1"
                ));
            }
        });
        Ok(ScenarioFixture {
        ...
        if has_evaluated_formulas(phase) {
            ...
            for row in [1, rows.max(2) / 2, rows] {
                ...
                out.push(ScenarioInvariant::CellEquals {
                    ...
                    row,
                    col: 2,
                    expected: numeric(value_for(key) * 2.0 + 1.0),
                });
            }
        }
        out
