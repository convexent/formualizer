...
pub struct S028LetLambdaFormulas {
    scale: ScaleState,
}

impl Default for S028LetLambdaFormulas {
    fn default() -> Self {
        Self::new()
    }
}

impl S028LetLambdaFormulas {
    pub fn new() -> Self {
        Self {
...
impl Scenario for S028LetLambdaFormulas {
    fn id(&self) -> &'static str {
        "s028-let-lambda-formulas"
    }

    fn description(&self) -> &'static str {
        "Formula family using LET local bindings equivalent to A-row times three."
    }

    fn tags(&self) -> &'static [ScenarioTag] {
        &[ScenarioTag::LetLambda, ScenarioTag::SingleCellEdit]
    }
    ...
    fn build_fixture(&self, ctx: &ScenarioBuildCtx) -> Result<ScenarioFixture> {
        ...
        write_workbook(&path, |book| {
            ...
            for r in 1..=rows {
                ...
                sheet
                    .get_cell_mut((2, r))
                    .set_formula(format!("=LET(x, A{r}, y, x*2, x+y)"));
            }
        });
        Ok(ScenarioFixture {
            path,
        ...
        if has_evaluated_formulas(phase) {
            ...
            for row in 1..=rows {
                invariants.push(ScenarioInvariant::CellEquals {
                    ...
                    row,
                    col: 2,
                    expected: numeric(data_value(row, rows, cycles) * 3.0),
                });
            }
        }
        invariants
